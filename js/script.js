...
window.addEventListener(
    ...
    () => {
        ...
        const sectionToHighlight = (
            inView.is(sections[sections.length - 1]) ?
            sections[sections.length - 1] :
            sections.find(section => inView.is(section))
        )
        const id = sectionToHighlight.getAttribute('id')
...
menuElements.forEach(menuElement => {
    menuElement.addEventListener(
        ...
        (e) => {
            e.preventDefault()
            const href = id = e.target.getAttribute('href')
            const elementToScroll = document.querySelector(href)
            window.scrollTo(0, elementToScroll.offsetTop - 150)
        }
    )
})
